...
function updatePackage($conn, $agentID, $input) {
    ...
    //  Optional fields 
    $description        = optionalString($input, "description",        null,    5000);
    $currency           = optionalString($input, "currency",           "ZAR",   10);
    $startDate          = optionalDate($input,   "startDate");
    $endDate            = optionalDate($input,   "endDate");
    $destinationCity    = optionalString($input, "destinationCity",    null,    100);
    $destinationCountry = optionalString($input, "destinationCountry", null,    100);
    ...
    if ($startDate && $endDate && $endDate < $startDate) {
        fail("endDate cannot be before startDate.");
    }
    ...
    // Bind type string — 10 SET params + 2 WHERE params = 12 total
    // s  title
    // s  description
    // d  pricePerPerson
    // s  currency
...
$stmt->bind_param(
    ...
    if ($stmt->affected_rows < 1) {
        $check = safePrepare($conn,
            "SELECT packageID FROM `Package` WHERE packageID = ? AND agentID = ?"
        );
        $check->bind_param("ii", $packageID, $agentID);
        $check->execute();

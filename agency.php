<?php
// Shelby Bodenstein, Idene Smit, Danel Steyn
...
//STATS FOR AGENCY DASHBOARD
function getDashboardStats($conn, $agentID) {
    // Total revenue from confirmed/completed bookings for this agent's packages
    $stmt = safePrepare($conn, "
        SELECT COALESCE(SUM(b.totalPrice), 0) AS totalRevenue
        FROM Booking b
    ...
    $stmt->execute();
    $revenueRow = $stmt->get_result()->fetch_assoc();

    // Average overall rating across all reviews
    $stmt = safePrepare($conn, "
        SELECT COALESCE(ROUND(AVG(r.overallScore), 1), 0) AS avgRating
        FROM Review r
    ...
    // 5 most recent reviews for this agent's packages
    $stmt = safePrepare($conn, "
        SELECT 
            r.reviewID,
            r.comment,
...
function requireAgentLogin() {
    ...
    $type = strtolower((string)($_SESSION["userType"] ?? ""));
...
function getComponentByID($conn, $agentID, $input) {
    ...
    } elseif ($component["componentType"] === "Flight") {
        $stmt = safePrepare($conn, "SELECT airlineName, flightNumber, departureAirport, arrivalAirport, departureTime, arrivalTime, cabinClass, baggageAllowance FROM `Flight` WHERE componentID = ?");
    } else {
...
function createComponent($conn, $agentID, $input) {
    ...
    if (!in_array($componentType, ["Accommodation", "Restaurant", "Excursion", "Flight"])) {
        fail("componentType must be Accommodation, Restaurant, Excursion, or Flight.");
    }
    ...
    try {
        ...
        if ($componentType === "Accommodation") {
            ...
            $stmt->execute();
        } elseif ($componentType === "Flight") {
            $airlineName = requiredString($input, "airlineName", 100);
            $flightNumber = optionalString($input, "flightNumber", null, 30);
            $departureAirport = requiredString($input, "departureAirport", 10);
            $arrivalAirport = requiredString($input, "arrivalAirport", 10);
            $departureTime = optionalString($input, "departureTime", null, 30);
            $arrivalTime = optionalString($input, "arrivalTime", null, 30);
            $cabinClass = optionalString($input, "cabinClass", "Economy", 50);
            $baggageAllowance = optionalString($input, "baggageAllowance", null, 100);
            $stmt = safePrepare($conn, "
                INSERT INTO `Flight` (componentID, airlineName, flightNumber, departureAirport, arrivalAirport, departureTime, arrivalTime, cabinClass, baggageAllowance)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->bind_param("issssssss", $componentID, $airlineName, $flightNumber, $departureAirport, $arrivalAirport, $departureTime, $arrivalTime, $cabinClass, $baggageAllowance);
            $stmt->execute();
        } else {
...
function updateComponent($conn, $agentID, $input) {
    ...
    try {
        ...
        $stmt->bind_param("i", $componentID);
        $stmt->execute();
        $stmt = safePrepare($conn, "DELETE FROM `Flight` WHERE componentID = ?");
        $stmt->bind_param("i", $componentID);
        $stmt->execute();
        ...
        if ($componentType === "Accommodation") {
            ...
            $cabinClass = optionalString($input, "cabinClass", "Economy", 50);
            $baggageAllowance = optionalString($input, "baggageAllowance", null, 100);
            $stmt = safePrepare($conn, "INSERT INTO `Flight` (componentID, airlineName, flightNumber, departureAirport, arrivalAirport, departureTime, arrivalTime, cabinClass, baggageAllowance) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("issssssss", $componentID, $airlineName, $flightNumber, $departureAirport, $arrivalAirport, $departureTime, $arrivalTime, $cabinClass, $baggageAllowance);
            $stmt->execute();
        } else {
            $duration = optionalString($input, "duration", null, 100);
...
function listGroupTrips($conn, $agentID) {
    $stmt = safePrepare($conn, "
        SELECT gt.groupTripID, gt.groupName, gt.currentMembers, gt.packageID,
               p.title AS title, p.destinationCity, p.destinationCountry
        FROM `GroupTrip` gt
...
// DEV MODE was to bypass Auth as an agent but added temporary Login 
// as well as a LoginAgent.php for correct verification
define("DEV_MODE", false);  // Set false — so real auth now
...
if ($action === "health") {
    respond(true, ["available_actions" => [
        ...
        "list_bookings","get_dashboard_stats"
...
switch ($action) {
    ...
    case "list_bookings":
    listBookings($conn, $agentID); break;
